...
app.post( '/', ( req, res ) => {
    ...
    {        
        ...
        data.valueTypes = qs.stringify(dailyDynamicValues.dailyDynamicValueTypes).replace('[', '').replace(']', '').replace(/[\d?][\d+]=/g,'').replace(/&/g,',')
    ...
    if (dynamicEpochValues.startTimestampUnix)
    {
        ...
        data.valueTypes = qs.stringify(dynamicEpochValues.dynamicValueTypes).replace('[', '').replace(']', '').replace(/[\d?][\d+]=/g,'').replace(/&/g,',')
        data.authenticationToken = authenticationToken        
        data.dataSources = dataSources
