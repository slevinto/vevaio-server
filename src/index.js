...
app.post( '/', ( req, res ) => {
    ...
    {        
        ...
        data.valueTypes = qs.stringify(dailyDynamicValues.dailyDynamicValueTypes).replace('[', '').replace(']', '').replace(/[0-9]+=/,'').replace(/&/g,',')
    ...
    if (dynamicEpochValues.startTimestampUnix)
    {
        ...
        data.valueTypes = qs.stringify(dynamicEpochValues.dynamicValueTypes).replace('[', '').replace(']', '').replace(/[0-9]+=/,'').replace(/&/g,',')
        data.authenticationToken = authenticationToken        
        data.dataSources = dataSources
